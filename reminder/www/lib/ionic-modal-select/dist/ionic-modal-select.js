...
var modalSelectTemplates = modalSelectTemplates || {};modalSelectTemplates['modal-template.html'] = ' <ion-modal-view class="ionic-select-modal" ng-class="::ui.modalClass">\n' +
    '    \n' +
    '    <ion-header-bar ng-class="::ui.headerFooterClass">\n' +
    '      <h1 class="title text-white">{{::ui.modalTitle}}</h1>\n' +
    '    </ion-header-bar>\n' +
    '\n' +
    ...
    '            <div ng-if="!ui.shortList">\n' +
    '                <div class="list" ng-if="showList" class="animate-if">\n' +
    '                    <div class="item item-text-wrap ink border-abu-bawah" collection-repeat="option in options track by $index" ng-click="setOption(option)" ng-class="{\'{{::ui.selectedClass}}\': compareValues(getSelectedValue(option), ui.value) }"> \n' +
    '                        <div compile="inner" compile-once="true"></div>\n' +
    '                    </div>\n' +
    '                </div>\n' +
    '            </div>\n' +
    '            <div ng-if="ui.shortList">\n' +
    '                <div class="list">\n' +
    '                    <div class="item item-text-wrap ink border-abu-bawah" ng-repeat="option in options track by $index" ng-click="setOption(option)" ng-class="{\'{{::ui.selectedClass}}\': compareValues(getSelectedValue(option), ui.value) }">\n' +
    '                        <div compile="inner" compile-once="true"></div>\n' +
    '                    </div>\n' +
    '                </div>\n' +
    '            </div>\n' +
    ...
    '\n' +
    '    <ion-footer-bar ng-class="::ui.headerFooterClass" >\n' +
    '        <div class="button-bar" style="padding:4px 4px 4px 24px;"><a class="button button-modal button-clear" ng-click="closeModal()">{{ui.cancelButton}}</a>\n' +
    '        <a ng-if="::!ui.hideReset" class="button button-modal button-clear" ng-click="unsetValue()">{{ui.resetButton}}</a></div>\n' +
    '    </ion-footer-bar>\n' +
    '\n' +
    '</ion-modal-view>\n' +
    '';
...
angular.module('ionic-modal-select', ['ionic-material'])
...
.directive('modalSelect', ['$ionicModal','$timeout', '$filter', '$parse','ionicMaterialInk', function ($ionicModal, $timeout, $filter, $parse,ionicMaterialInk) {
    return {
        restrict: 'A',
        require : 'ngModel',
        ...
        link: function (scope, iElement, iAttrs, ngModelController, transclude) {
            ...
            scope.ui = {
                ...
                resetButton : iAttrs.resetButton || 'Kosongkan',
                cancelButton : iAttrs.cancelButton || 'Batal',
                loadListMessage : iAttrs.loadListMessage || 'Loading',
                modalClass : iAttrs.modalClass || '',
                headerFooterClass : iAttrs.headerFooterClass || 'bar-stable bg-biru',
                value  : null,
